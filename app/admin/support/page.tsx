...
export default function AdminSupportDashboard() {
    ...
    const validateAdmin = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const ok = res.ok;
          setAuthed(ok);
          if (!ok) localStorage.removeItem('sm_admin_token');
          setAuthChecked(true);
        }
      } catch {
        if (!cancelled) {
  ...
  if (!authChecked) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#060b14', color: '#cbd5e1', fontFamily: "'DM Sans', sans-serif" }}>
  ...
  if (!authed) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#060b14', padding: 20, fontFamily: "'DM Sans', sans-serif" }}>
