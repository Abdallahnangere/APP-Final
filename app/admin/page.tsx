...
export default function AdminPage() {
  ...
  const fetchCalculator = async () => {
      ...
      try {
          ...
          if (data.report) setSalesReport(data.report);
      } catch (e) { alert("Calc Error"); }
        ...
        <main className="flex-1 lg:ml-72 p-10 overflow-y-auto h-screen">
            <header className="flex justify-between items-center mb-10">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {/* Restore Analytics Button */}
                    <button onClick={fetchData} title="Sync/Restore Analytics" className="p-3 bg-white border border-slate-100 rounded-2xl shadow-sm hover:bg-slate-50 transition-all flex items-center gap-2 group active:scale-95">
            ...
            {view === 'calculator' && (
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div>
                            <h3 className="font-black text-lg uppercase tracking-tight">Sales Ledger & Settlement</h3>
                            <p className="text-[10px] text-slate-400 uppercase tracking-widest">Aggregated Units • Auto-Fee Calculation (2%) • VAT on Fee (7.5%)</p>
                        </div>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50/50 border-b border-slate-100">
                                <tr>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400">Item Details</th>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400">Unit Price</th>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400">Units Sold</th>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400">Total Revenue</th>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400 text-red-400">Platform Fee (2%)</th>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400 text-red-400">VAT on Fee (7.5%)</th>
                                    <th className="p-6 text-[10px] font-black uppercase text-slate-400 text-green-600 text-right">Net Settlement</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {salesReport.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-6">
                                            <p className="font-black text-slate-900 text-sm uppercase">{row.name}</p>
                                            <p className="text-[10px] text-slate-400 font-bold uppercase">{row.type}</p>
                                        </td>
                                        <td className="p-6 font-mono text-xs font-bold text-slate-600">{formatCurrency(row.unitPrice)}</td>
                                        <td className="p-6">
                                            <span className="bg-slate-100 px-3 py-1 rounded-lg text-xs font-black">{row.unitsSold} Units</span>
                                        </td>
                                        <td className="p-6 font-mono text-sm font-black text-slate-900">{formatCurrency(row.totalRevenue)}</td>
                                        <td className="p-6 font-mono text-xs font-bold text-red-500">-{formatCurrency(row.fee)}</td>
                                        <td className="p-6 font-mono text-xs font-bold text-red-500">-{formatCurrency(row.vatOnFee)}</td>
                                        <td className="p-6 text-right">
                                            <span className="bg-green-100 text-green-700 px-4 py-2 rounded-xl text-xs font-black font-mono">
                                ...
                                {salesReport.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="p-12 text-center text-slate-400 text-xs font-black uppercase tracking-widest">No Sales Data Available</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            {view === 'agents' && (
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="p-8 border-b border-slate-100 bg-slate-50/50">
                        <h3 className="font-black text-lg uppercase tracking-tight">Registered Agents</h3>
                        <p className="text-[10px] text-slate-400 uppercase tracking-widest">Wallet Balances • Banking Details • Activity</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8">
                        {agents.map(agent => (
                            <div key={agent.id} className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 flex flex-col justify-between group hover:border-blue-200 transition-all">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-black text-slate-900 uppercase text-sm">{agent.firstName} {agent.lastName}</h4>
                                            <p className="text-[11px] text-slate-500 font-bold">{agent.phone}</p>
                                        </div>
                                    </div>
                                    <span className="bg-white px-3 py-1 rounded-lg text-[10px] font-black uppercase shadow-sm border border-slate-100">
                                        {agent._count.transactions} Txns
                                    </span>
                                </div>
                                
                                <div className="space-y-4">
                                    <div className="bg-white p-4 rounded-xl border border-slate-100">
                                        <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-1">Wallet Balance</p>
                                        <p className="text-2xl font-black text-slate-900 tracking-tighter">{formatCurrency(agent.balance)}</p>
                                    </div>
                                    <div className="flex justify-between items-center text-[10px] font-mono text-slate-500 uppercase">
                                        <span>{agent.flwBankName}</span>
                                        <span className="font-bold text-slate-900">{agent.flwAccountNumber}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
