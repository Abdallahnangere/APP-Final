...
export async function GET(req: NextRequest) {
  ...
  const [totalTxns] = await sql`SELECT COUNT(*) as c, COALESCE(SUM(amount),0) as vol FROM transactions WHERE status='success' AND created_at > NOW() - INTERVAL '${sql.unsafe(String(days))} days'`;
  const [totalDeposits] = await sql`SELECT COALESCE(SUM(amount),0) as vol FROM deposits WHERE created_at > NOW() - INTERVAL '${sql.unsafe(String(days))} days'`;
  ...
  const salesData = await sql`
    ...
    AND t.created_at > NOW() - INTERVAL '${sql.unsafe(String(days))} days'
  ...
  const dailyTxns = await sql`
    ...
    FROM transactions WHERE status='success' AND created_at > NOW() - INTERVAL '${sql.unsafe(String(days))} days'
