...
export const Track: React.FC<TrackProps> = ({ onBack }) => {
  ...
  const handleDownloadReceipt = async (tx: Transaction) => {
      setReceiptTx(tx);
      // Wait for React to render the hidden receipt component
      setTimeout(async () => {
  ...
  return (
    <div className="p-6 pb-32 min-h-screen">
      <div className="flex items-center gap-3 mb-8">
          {onBack && (
            <button onClick={onBack} className="bg-slate-100 p-2 rounded-xl text-slate-500 font-bold text-xs uppercase tracking-widest hover:bg-slate-200 transition-colors">
                Back
            </button>
          )}
          <div>
            <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-none">Order Tracking</h1>
